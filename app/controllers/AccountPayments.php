...
class AccountPayments extends Controller {
    ...
    public function index() {
        ...
        $payments = [];
        $payments_result = Database::$database->query("SELECT `payments`.*, `packages`.`name` AS `package_name` FROM `payments` LEFT JOIN `packages` ON `payments`.`package_id` = `packages`.`package_id` WHERE `user_id` = {$this->user->user_id} ORDER BY `id` DESC LIMIT {$paginator->getSqlOffset()}, {$paginator->getItemsPerPage()}");
