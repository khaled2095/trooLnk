    ...
    public function index() {
        ...
        if(!empty($_POST)) {
            ...
            if(empty($_SESSION['error'])) {
                ...
                $stmt = Database::$database->prepare("INSERT INTO `users` (`password`, `email`, `email_activation_code`, `name`, `package_id`, `package_expiration_date`, `package_settings`, `active`, `date`, `ip`, `last_user_agent`, `total_logins`, `timezone`, `language`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param('ssssssssssssss', $password, $_POST['email'], $email_code, $_POST['name'], $package_id, $package_expiration_date, $package_settings, $active, \Altum\Date::$date, $ip, $last_user_agent, $total_logins, $timezone, $language);
                $stmt->execute();
