...
class AdminPayments extends Controller {
    ...
    public function read() {
        ...
        $datatable->set_accepted_columns(['type', 'email', 'name', 'amount', 'date', 'user_email', 'actions']);
        ...
        while($row = $result->fetch_object()):
            ...
            <div class="d-flex flex-column">
                <span>' . $this->language->pay->custom_package->{$row->type . '_type'} . '</span>
                <span class="text-muted">' . $this->language->pay->custom_package->{strtolower($row->processor)} . '</span>
            </div>
            ';
            ...
            $row->actions = include_view(THEME_PATH . 'views/admin/partials/admin_payment_dropdown_button.php', [
                ...
                'processor' => strtolower($row->processor),
                'status' => $row->status
            ]);
    ...
    public function approve() {
        ...
        if(empty($_SESSION['error'])) {
            $payment = Database::get(['package_id', 'frequency', 'email', 'code', 'payment_proof', 'payer_id'], 'payments', ['id' => $payment_id]);
            $package = (new \Altum\Models\Package(['settings' => $this->settings]))->get_package_by_id($payment->package_id);

            /* Give the package to the user */
            ...
            $email_template = get_email_template(
                ...
                [
                    '{{PACKAGE_EXPIRATION_DATE}}' => Date::get($package_expiration_date, 2),
