    ...
    public function index() {
        ...
        if(!empty($_POST)) {
            ...
            if(empty($_SESSION['error'])) {
                ...
                $stmt = Database::$database->prepare("INSERT INTO `users` (`password`, `email`, `email_activation_code`, `name`, `package_id`, `package_expiration_date`, `package_settings`, `language`, `timezone`, `active`, `date`, `ip`, `country`, `last_user_agent`, `total_logins`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param('sssssssssssssss', $password, $_POST['email'], $email_code, $_POST['name'], $package_id, $package_expiration_date, $package_settings, Language::$language, $timezone, $active, \Altum\Date::$date, $ip, $country, $user_agent, $total_logins);
                $stmt->execute();
